...
try:
 ...
 class Cargo(Base):
    ...
    def __repr__(self):
     return f'<Cargo nivel={self.nivel}>' # monstrar o titulo

 # Popular cargos (se ainda não tiver dados)
 if db.query(Departamento).count() == 0: # qual modelo verificar?
    db.add_all([
       Cargo(titulo='Desenvolvedor', nivel='Junior', salario_min=2500, salario_max=4000),
       Cargo(titulo='Desenvolvesor', nivel='Pleno', salario_min=4000, salario_max=7000),
    ...
    for c in todos:
       ...
       if designer:
          print(f'Designer encontrado: faixa R$ {designer.salario_min} -R$ {designer.salario_max}') # qual campo?
